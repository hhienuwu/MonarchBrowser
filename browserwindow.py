...
try:
    ...
    ctypes.windll.user32.MessageBoxW( 0,"Không thể import các thư viện của trình duyệt, xin vui lòng cài lại trình duyệt", "Monarch Browser", 1)

def about():
    ctypes.windll.user32.MessageBoxW( 0,"Monarch là một trình duyệt nhẹ, đơn giản nhưng bảo mật được phát triển bởi tôi. Bạn có thể sử dụng trình duyệt này làm trình duyệt chính của bạn hoặc chỉ đơn giản là tải Monarch về cho vui. Nhưng miễn sao là bạn thích là được. Nếu bất kỳ vấn đề nào như lỗi, thắc mắc,... xin đừng ngần ngại mà hãy gửi mail cho tôi: trdahien2011@gmail.com.", "About Monarch Browser", 1)

def incognito():
    try:
        os.startfile(".\\incognito\\main.exe")
    except:
        ctypes.windll.user32.MessageBoxW( 0,"Không tìm thấy file Monarch Incognito, xin vui lòng cài lại trình duyệt", "Monarch Browser", 1)
...
def passstrength():
    try:
        ...
        url = easygui.passwordbox("Nhập Password cần kiểm tra\n\nPassword của bạn được bảo mật. Nếu không tin, bạn có \nthể kiểm tra mã nguồn của Monarch hoặc liên hệ trdahien2011@gmail.com")
        password = url

        is_valid = validate_password(password)

        if is_valid:
            ctypes.windll.user32.MessageBoxW( 0,"Mật khẩu của bạn rất mạnh! Tiếp tục phát huy nhé!", "Monarch Password Checker", 1)
        else:
            ctypes.windll.user32.MessageBoxW( 0,"Đã đến lúc đổi password rồi! Password của bạn rất yếu, bạn nên đặt password có tối thiểu 8 kí tự, có chữ viết hoa và viết thường, có cả số và kí tự đặc biệt. Cũng không nên đặt password phổ biến / dễ đoán ", "Monarch Password Checker", 1)
    except:
        ctypes.windll.user32.MessageBoxW( 0,"Không thể khởi chạy Monarch Password Checker. Vui lòng cài lại trình duyệt", "Monarch Browser", 1)

def chatgpt():
    try:
        from PyQt5 import QtGui
        from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QLabel, QLineEdit, QTextEdit, QPushButton, QVBoxLayout
        openai.api_key = "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"
        ...
        window.setWindowIcon(QtGui.QIcon('.\\data\\logochatgpt.png'))
        ...
        def send_message():
            ...
            try:
                ...
                ctypes.windll.user32.MessageBoxW( 0,"Không thể kết nối tới server của OpenAI, xin vui lòng kiểm tra lại internet.", "Monarch Browser", 1)
        ...
        ctypes.windll.user32.MessageBoxW( 0,"Không thể khởi chạy ChatGPT, xin vui lòng kiểm tra lại mạng.", "Monarch Browser", 1)
def antivirus():
    try:
        import base64
        import tkinter as tk
        ...
        ctypes.windll.user32.MessageBoxW( 0,"Không thể khởi chạy Monarch Antivirus, xin vui lòng kiểm tra lại mạng.", "Monarch Browser", 1)
def urlchecker():
    try:
        ...
        ctypes.windll.user32.MessageBoxW( 0,"Không thể khởi chạy Monarch Url Checker, xin vui lòng kiểm tra lại mạng.", "Monarch Browser", 1)
...
try:
    ...
    class BrowserWindow(QMainWindow):
        ...
        def __init__(self, browser, profile, forDevTools,):
            ...
            self._home_icon = QIcon(".\\data\\home.png")
            ...
            if not forDevTools:
                ...
                mb.addMenu(self.create_settings_menu())
        ...
        def create_help_menu(self):
            ...
            a_passwordstrength = help_menu.addAction("Kiểm tra độ mạnh của password")
            a_passwordstrength.triggered.connect(passstrength)
            return help_menu
        ...
        def create_settings_menu(self):
            settings_menu = QMenu("Cài Đặt")
            a_about = settings_menu.addAction("Về Monarch")
            a_about.triggered.connect(about)
        ...
        def create_tool_bar(self):
            ...
            self._home_action.setIcon(QIcon(".\\data\\home.png"))
            self._home_action.setToolTip("Trang chủ")
            self._home_action.triggered.connect(self._go_home)
            navigation_bar.addAction(self._home_action)
    ...
    ctypes.windll.user32.MessageBoxW( 0,"Không thể khởi chạy trình duyệt, xin vui lòng cài lại trình duyệt", "Monarch Browser", 1)