...
try:
    ...
    def question_for_feature(feature):
        if feature == QWebEnginePage.Geolocation:
            return "Bạn có cho phép %1 biết địa chỉ hiện tại của bạn không?"
        if feature == QWebEnginePage.MediaAudioCapture:
            return "Bạn có cho phép %1 truy cập vào mic của bạn?"
        if feature == QWebEnginePage.MediaVideoCapture:
            return "Bạn có cho phép %1 truy cập vào camera của bạn?"
        if feature == QWebEnginePage.MediaAudioVideoCapture:
            return "Bạn có cho phép %1 truy cập vào cả mic và camera của bạn?"
        if feature == QWebEnginePage.MouseLock:
            return "Bạn có cho phép %1 khóa con trỏ chuột của bạn?"
        if feature == QWebEnginePage.DesktopVideoCapture:
            return "Bạn có cho phép %1 chụp màn hình máy tính bạn?"
        if feature == QWebEnginePage.DesktopAudioVideoCapture:
            return "Bạn có cho phép %1 quay lại video và mic của bạn?"
        if feature == QWebEnginePage.Notifications:
            return "Bạn có cho phép %1 hiện thông báo trên màn hình desktop?"
        return ""
    ...
    class WebView(QWebEngineView):
        ...
        def __init__(self, parent=None):
            ...
            self._error_icon = QIcon(".\\data\\error.png")
            self._loading_icon = QIcon(".\\data\\refresh.png")
            self._default_icon = QIcon(".\\data\\html.png")
        ...
        def _render_process_terminated(self, termStatus, statusCode):
            ...
            if termStatus == QWebEnginePage.NormalTerminationStatus:
                status = "Quá trình render bình thường"
            elif termStatus == QWebEnginePage.AbnormalTerminationStatus:
                status = "Quá trình render không bình thường"
            elif termStatus == QWebEnginePage.CrashedTerminationStatus:
                status = "Quá trình render bị sập"
            elif termStatus == QWebEnginePage.KilledTerminationStatus:
                status = "Quá trình render bị tắt"

            m = f"Quá trình render bị lỗi với mã {statusCode:#x}\nBạn có muốn load lại trang?"
        ...
        def contextMenuEvent(self, event):
            ...
            if inspect_action in actions:
                inspect_action.setText("Kiểm tra")
            else:
                vs = self.page().action(QWebEnginePage.ViewSource)
                if vs not in actions:
                    menu.addSeparator()

                action = menu.addAction("Kiểm tra trong cửa sổ mới")
        ...
        def handle_certificate_error(self, error):
            ...
            dialog.setWindowTitle("Lỗi chứng chỉ")
        ...
        def handle_authentication_required(self, requestUrl, auth):
            ...
            realm = auth.realm()
            m = f'Nhập tên người dùng và password cho "{realm}" ở {url_str}'
        ...
        def handle_proxy_authentication_required(self, url, auth, proxyHost):
            ...
            password_dialog.m_infoLabel.setText(f'Kết nối vào proxy "{proxy}" dùng:')
            password_dialog.m_infoLabel.setWordWrap(True)
        ...
        def handle_register_protocol_handler_requested(self, request):
            ...
            m = f"Cho phép {host} mở tất cả {request.scheme()} link?"
            answer = QMessageBox.question(self.window(), "Cần quyền truy cập", m)
        ...
        def handle_file_system_access_requested(self, request):
            ...
            if type == QWebEngineFileSystemAccessRequest.Read:
                ...
                access_type = "ghi và viết"
            host = request.origin().host()
            path = request.filePath().toString()
            t = "File hệ thống cần quyền truy cập"
            m = f"Cho phép {host} {access_type} truy cập vào {path}?"
    ...
    ctypes.windll.user32.MessageBoxW( 0,"Không thể hiển thị trang web, xin vui lòng cài lại trình duyệt", "Monarch Browser", 1)