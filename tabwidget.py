...
try:
    ...
    ctypes.windll.user32.MessageBoxW( 0,"Không thể import các thư viện từ tab của trình duyệt, xin vui lòng cài lại trình duyệt", "Monarch Browser", 1)
...
try:
    class TabWidget(QTabWidget):
        ...
        def handle_context_menu_requested(self, pos):
            menu = QMenu()
            menu.addAction("Tab Mới", QKeySequence.AddTab, self.create_tab)
            index = self.tabBar().tabAt(pos)
            if index != -1:
                action = menu.addAction("Nhân Bản Tab")
                action.triggered.connect(partial(self.clone_tab, index))
                menu.addSeparator()
                action = menu.addAction("Tắt Tab")
                action.setShortcut(QKeySequence.Close)
                action.triggered.connect(partial(self.close_tab, index))
                action = menu.addAction("Tắt Tab Khác")
                action.triggered.connect(partial(self.close_other_tabs, index))
                menu.addSeparator()
                action = menu.addAction("Tải Lại Tab")
                action.setShortcut(QKeySequence.Refresh)
                action.triggered.connect(partial(self.reload_tab, index))
            else:
                menu.addSeparator()

            menu.addAction("Tải Lại Tất Cả Tab", self.reload_all_tabs)
        ...
        def create_background_tab(self):
            ...
            index = self.addTab(web_view, "Tab Mới")
    ...
    ctypes.windll.user32.MessageBoxW( 0,"Không thể khởi chạy phần tab của trình duyệt, xin vui lòng cài lại trình duyệt", "Monarch Browser", 1)
